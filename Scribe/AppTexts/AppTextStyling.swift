...
func addHyperLinks(originalText: String, links: [String: String], fontSize: CGFloat)
    -> NSMutableAttributedString {
    ...
    for (hyperLink, urlString) in links {
        ...
        attributedOriginalText.addAttribute(
            NSAttributedString.Key.link, value: urlString, range: linkRange
        )
        attributedOriginalText.addAttribute(
            NSAttributedString.Key.paragraphStyle, value: style, range: fullRange
        )
    }
...
func addTabStops(attributedOriginalText: NSMutableAttributedString, links: [String])
    -> NSMutableAttributedString {
...
func getArrowIcon(fontSize: CGFloat) -> NSAttributedString {
    ...
    let selectArrowIconConfig = UIImage.SymbolConfiguration(
        pointSize: fontSize, weight: .medium, scale: .medium
    )
...
func getCenteredImagesForWikimediaAndScribe(imageString: String, imageWidth: CGFloat)
    -> NSMutableAttributedString {
    if let image = UIImage(named: imageString) {
        ...
        imageAttachment.bounds = CGRect(
            origin: .zero, size: CGSize(width: imageWidth, height: imageHeight)
        )
...
func switchAttachmentOnThemeChange(for attributedString: NSAttributedString)
    -> NSMutableAttributedString {
    let mutableString = NSMutableAttributedString(attributedString: attributedString)

    mutableString.enumerateAttribute(
        .attachment, in: NSRange(location: 0, length: mutableString.length), options: []
    ) { attachment, range, _ in
        guard let attachment = attachment as? NSTextAttachment,
              let asset = attachment.image?.imageAsset
        else { return }
...
func setWikimediaAndScribe(text: [String], fontSize: CGFloat, imageWidth: CGFloat)
    -> NSMutableAttributedString {
...
func pairLinks(_ linkList: [String]) -> [String: String] {
    ...
    for hyperlink in linkList {
        if hyperlink.contains("@"),
           hyperlink.range(of: #"https?://"#, options: .regularExpression) == nil {
