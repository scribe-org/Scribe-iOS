...
the iOS application "Scribe - Language Keyboards" (SERVICE) as an open-source application. This SERVICE is provided by SCRIBE at no cost and is intended for use as is.
...
"""
  ...
  override func viewDidLoad() {
    ...
    navigationItem.title = "Privacy test"
    headingLabel.text = "Test heading"
  ...
  func setupInformationPage() {
    containerView.backgroundColor = .clear
    
    
    textView.backgroundColor = UIColor(named: "commandBar")
    applyCornerRadius(elem: textView, radius: 16)
    applyShadowEffects(elem: textView)
  }
}
