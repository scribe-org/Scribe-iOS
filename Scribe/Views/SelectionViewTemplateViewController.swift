...
final class SelectionViewTemplateViewController: BaseTableViewController {
    ...
    func configureTable(
        for tableData: [ParentTableCellModel], parentSection: Section, langCode: String
    ) {
...
extension SelectionViewTemplateViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if newLang != oldLang {
            ...
            showPopup(
                oldLang: oldLang, newLang: newLang, oldIndexPath: oldIndexPath,
    ...
    private func showPopup(
        oldLang: String, newLang: String, oldIndexPath: IndexPath?, newIndexPath _: IndexPath,
        ...
        let infoText = NSLocalizedString(
            ...
            value:
            "You've changed your source translation language. Would you like to download new data so that you can translate from {source_language}?",
            comment: ""
        )
        ...
        func confirmDownload() {
            dismiss(animated: true) {
                ...
                DownloadStateManager.shared.handleDownloadAction(
                    key: self.langCode, forceDownload: true
                )

                guard let tabBarController = self.tabBarController else { return }

                guard
                    let installationNavController = tabBarController.viewControllers?[0]
        ...
        let popupView = ConfirmTranslationSource(
            infoText: infoText.replacingOccurrences(
                of: "{source_language}", with: localizedNewSourceLanguage
            ).replacingOccurrences(of: "{source_language}", with: localizedNewSourceLanguage),
            changeButtonText: changeButtonText.replacingOccurrences(
                of: "{source_language}", with: localizedOldSourceLanguage
            ),
