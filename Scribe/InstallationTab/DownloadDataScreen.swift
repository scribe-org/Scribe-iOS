...
struct CheckDataSpinner: View {
    ...
    var body: some View {
        ZStack {
            switch state {
            case .idle:
                // Empty gray circle as idle state.
                Circle()
            ...
            case .checking:
                ...
                ZStack {
                    ...
                    Circle()
                        .trim(from: 0, to: 0.7)
                        .stroke(
                            Color("scribeCTA"), style: StrokeStyle(lineWidth: 2.5, lineCap: .round)
                        )
                        .frame(width: 28, height: 28)
...
struct EmptyStateView: View {
    private var noKeyboardText = NSLocalizedString(
        ...
        value:
        "You currently do not have any Scribe keyboard installed. Please click the Install keyboards button below to install a Scribe keyboard and then come back to download the needed data.",
        comment: ""
    )
...
struct LanguageListView: View {
    ...
    private func handleUpdateAllLanguages() {
        let toDownload = stateManager.downloadStates.keys.filter {
            stateManager.downloadStates[$0] != .updated
                && stateManager.downloadStates[$0] != .downloading
        }
                    ...
                    VStack(spacing: 0) {
                        ...
                        ForEach(Array(languages.enumerated()), id: \.offset) { index, section in
                            ...
                            LanguageDownloadCard(
                                ...
                                action: {
                                    handleButtonClick(
                                        targetLang: section.sectionTitle, langCode: langCode
                                    )
                                }
                            )
    ...
    private var confirmDialogView: some View {
        ...
        let selectedSourceLang =
            userDefaults.string(forKey: languageCode + "TranslateLanguage") ?? "en"
        ...
        return ConfirmTranslationSource(
            infoText: NSLocalizedString(
                ...
                value:
                "The data you will download will allow you to translate from {source_language} to {target_language}. Do you want to change the language you'll translate from?",
                comment: ""
            )
...
struct DownloadDataScreen: View {
    ...
    private func initializeLanguageStates() {
        ...
        let languageKeys = languages.compactMap { section -> String? in
            if case let .specificLang(abbreviation) = section.sectionState {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                UpdateDataCardView(
                    languages: languages, onInitializeStates: initializeLanguageStates
                )
                LanguageListView(
                    onNavigateToTranslationSource: onNavigateToTranslationSource,
                    languages: languages
                )
            }
            .padding()
