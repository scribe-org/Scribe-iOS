...
class LanguageDBManager {
    ...
    private func queryDBRow(query: String, outputCols: [String], args: StatementArguments)
        -> [String] {
    ...
    ///   - outputCols: the columns from which the values should come.
    ///   - args: arguments to pass to `query`.
    private func queryDBRows(query: String, outputCols: [String], args: StatementArguments)
        -> [String] {
        var outputValues = [String]()
        do {
...
extension LanguageDBManager {
    ...
    func queryAllPluralForms() -> [String]? {
        ...
        guard let queryInfo = PluralManager.shared.buildAllPluralsQuery(contract: contract)
        else {
            return nil
        }
    ...
    func queryPrepForm(of word: String) -> [String] {
        ...
        guard hasGrammaticalCaseColumn()
        else {
            return [""]
        }
    ...
    func queryVerb(of word: String, identifierColumn: String? = nil, with outputCols: [String])
        -> [String] {
        let columnName =
            identifierColumn ?? ((controllerLanguage == "Swedish") ? "verb" : "infinitive")
        let query = """
        SELECT
          *
