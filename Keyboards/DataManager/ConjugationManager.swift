...
class ConjugationManager {
    ...
    func getConjugations(
        ...
        guard let conjugations = contract.conjugations
        else {
            return nil
        }
    ...
    private func parseComplexForm(
        ...
        guard let regex = try? NSRegularExpression(pattern: pattern),
              let match = regex.firstMatch(
                  in: columnName, range: NSRange(columnName.startIndex..., in: columnName)
              )
        else {
            return ""
        }
        ...
        let mainColumn = columnName.replacingOccurrences(
        ...
        if auxWords.count > 1 {
            ...
            if let auxVerbId = LanguageDBManager.shared.queryVerb(of: verb, with: [auxColumn])
                .first,
                !auxVerbId.isEmpty {
                ...
                if auxConjugated?.isEmpty ?? true {
                    auxConjugated =
                        LanguageDBManager.shared.queryVerb(
                            of: auxVerbId,
                            with: [targetForm]
                        ).first
                }
