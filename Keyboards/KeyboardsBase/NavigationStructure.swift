...
struct NavigationBuilder {
  ...
  static func buildConjugationCases(
    ...
    guard let conjugationData = ConjugationManager.shared.getConjugations(
      verb: verb,
      language: language
    ) else {
      return nil
    ...
    for (tenseTitle, conjugationTypes) in conjugationData {
      if conjugationTypes.count == 1 {
        ...
        tenseLevels.append(NavigationLevel(
        ...
        for (typeTitle, forms) in conjugationTypes {
          ...
          typeOptions.append((label: typeTitle, node: .nextLevel(formLevel, displayValue: displayValue)))
        }
