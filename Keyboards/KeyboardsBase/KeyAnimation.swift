...
func leftKeyPopPath(
    ...
    path.addLine(
        to: CGPoint(x: horizStart + (keyWidth * (1 + widthMultiplier * 0.35)), y: maxHeight)
    )
    path.addCurve(
        to: CGPoint(
            x: horizStart + (keyWidth * (1 + widthMultiplier)), y: heightBeforeTopCurves * 1.15
        ),
        controlPoint1: CGPoint(
            x: horizStart + (keyWidth * (1 + widthMultiplier * 0.75)), y: maxHeight
        ),
    ...
    path.addLine(
        to: CGPoint(
            x: horizStart + (keyWidth * (1 + widthMultiplier)), y: vertStart - (keyHeight * 1.3)
        )
    )
...
func getKeyPopPath(key: UIButton, layer: CAShapeLayer, char: String, displayChar: String) {
    // We use idx to differentiate between keys that share the same character, for example "§" on Swedish iPad keyboards.
...
func setPadKeyPopCharSize(char: String) {
    ...
    } else if keyboardState == .letters,
              shiftButtonState == .shift || shiftButtonState == .capsLocked {
        if isLandscapeView {
            keyPopChar.font = .systemFont(ofSize: letterKeyWidth / 2.5)
            keyHoldPopChar.font = .systemFont(ofSize: letterKeyWidth / 2.5)
        } else {
