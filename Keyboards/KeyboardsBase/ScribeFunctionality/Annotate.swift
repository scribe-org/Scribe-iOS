...
let prepAnnotationConversionDict = [
    "German": ["Acc": "Akk"],
    "Russian": [
        "Acc": "Вин", "Dat": "Дат", "Gen": "Род", "Loc": "Мес", "Pre": "Пре", "Ins": "Инс"
    ]
]
...
func wordAnnotation(wordToAnnotate: String, KVC: KeyboardViewController) {
    ...
    if wordToAnnotate == "Scribe" || wordToAnnotate == "scribe" {
        ...
        setBtn(
            btn: annotationBtn, color: commandKeyColor, name: "ScribeAnnotation",
        ...
        if numAnnotations > 0 {
            ...
            for i in 0 ..< numAnnotations {
                ...
                if ["feminine", "masculine", "neuter", "common"].contains(
                    annotationToDisplay.lowercased()
                ) {
                ...
                if nounFormToColorDict.keys.contains(annotationToDisplay) {
                    if numAnnotations > 3 {
                        annotationBtn.setAnnotationSize(
                            ...
                            fontSize: annotationHeight * 0.4
                        )
                    } else if numAnnotations > 2 {
                        annotationBtn.setAnnotationSize(
                            width: annotationWidth, height: annotationHeight,
                            fontSize: annotationHeight * 0.55
                        )
                    } else {
                        annotationBtn.setAnnotationSize(
                            width: annotationWidth, height: annotationHeight,
                            fontSize: annotationHeight * 0.6
                        )
                    }
                } else {
                    if numAnnotations > 3 {
                ...
                if nounFormToColorDict.keys.contains(annotationToDisplay) {
                    if nounAnnotationConversionDict[controllerLanguage] != nil {
                        if nounAnnotationConversionDict[controllerLanguage]?[annotationsToAssign[i]]
                            != nil {
                            annotationToDisplay =
                                nounAnnotationConversionDict[controllerLanguage]?[
                                    annotationsToAssign[i]
                                ] ?? ""
                        }
                    }
                } else {
                    if prepAnnotationConversionDict[controllerLanguage] != nil {
                        if prepAnnotationConversionDict[controllerLanguage]?[annotationsToAssign[i]]
                            != nil {
                            annotationToDisplay =
                                prepAnnotationConversionDict[controllerLanguage]?[
                                    annotationsToAssign[i]
                                ] ?? ""
                        }
                    }
                ...
                if i != 0 {
                    annotationSep = UIView(
                        frame: CGRect(
                            x: annotationBtn.frame.minX, y: annotationBtn.frame.minY, width: 1,
                            height: annotationBtn.frame.height
                        )
                    )
                    annotationSep.isUserInteractionEnabled = false
                    annotationSep.backgroundColor =
...
func typedWordAnnotation(KVC: KeyboardViewController) {
    guard let documentContextBeforeInput = proxy.documentContextBeforeInput,
    ...
    guard let lastWordTyped = wordsTyped.secondToLast()
    else {
        return
    }
            ...
            if index == 0 {
                annotationBtn.setAnnotationLoc(
                    minX: KVC.translateKey.frame.origin.x
                        ...
                        - (annotationFieldWidth / 2)
                        + (annotationWidth * CGFloat(i)),
                    maxY: KVC.translateKey.frame.origin.y + KVC.translateKey.frame.height - KVC
                        .scribeKey
                        .frame.height * 0.1
                )
            } else if index == 1 {
                annotationBtn.setAnnotationLoc(
                    minX: KVC.conjugateKey.frame.origin.x
                        ...
                        - (annotationFieldWidth / 2)
                        + (annotationWidth * CGFloat(i)),
                    maxY: KVC.conjugateKey.frame.origin.y + KVC.conjugateKey.frame.height - KVC
                        .scribeKey
                        .frame.height * 0.1
                )
            } else if index == 2 {
                annotationBtn.setAnnotationLoc(
                    minX: KVC.pluralKey.frame.origin.x
                        ...
                        - (annotationFieldWidth / 2)
                        + (annotationWidth * CGFloat(i)),
                    maxY: KVC.pluralKey.frame.origin.y + KVC.pluralKey.frame.height - KVC.scribeKey
                        .frame
                        .height * 0.1
                )
            }
...
class Annotation: UIButton {
    ...
    func style() {
        ...
        setTitleColor(
            UITraitCollection.current.userInterfaceStyle == .light ? keyColor : specialKeyColor,
            for: .normal
        )
        if activateAnnotationBtn {
