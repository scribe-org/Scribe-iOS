...
func setAlternatesPathState(
    ...
    if side == "left" {
        ...
        alternatesLongWidth =
            horizStart + keyWidth - CGFloat(keyWidth * numAlternates + (3.0 * numAlternates) + 8.0)
    }
...
func alternateKeysPathLeft(
    ...
    setAlternatesPathState(
        startY: startY, keyWidth: keyWidth, keyHeight: keyHeight, numAlternates: numAlternates,
        side: "left"
    )
    ...
    path.addCurve(
        ...
        controlPoint2: CGPoint(
            x: horizStart - (keyWidth * widthMultiplier), y: vertStart - (keyHeight * 1.05)
        )
    )

    // Path up and curve right past the top left.
    path.addLine(
        to: CGPoint(x: horizStart - (keyWidth * widthMultiplier), y: heightBeforeTopCurves)
    )
    path.addCurve(
        to: CGPoint(x: horizStart + (keyWidth * 0.075), y: maxHeight),
        controlPoint1: CGPoint(
            x: horizStart - (keyWidth * widthMultiplier), y: maxHeightCurveControl
        ),
    ...
    path.addCurve(
        to: CGPoint(
            x: alternatesLongWidth - maxWidthCurveControl, y: vertStart - (keyHeight * 0.95)
        ),
        controlPoint1: CGPoint(x: alternatesLongWidth, y: vertStart - (keyHeight * 1.05)),
        controlPoint2: CGPoint(
            x: alternatesLongWidth - (keyWidth * 0.2), y: vertStart - (keyHeight * 0.95)
        )
    )
    ...
    path.addCurve(
        ...
        controlPoint1: CGPoint(
            x: horizStart + (keyWidth * 1.05), y: vertStart - (keyHeight * 0.95)
        ),
...
func alternateKeysPathRight(
    ...
    path.addCurve(
        ...
        controlPoint2: CGPoint(
            x: horizStart + (keyWidth * (1 + widthMultiplier)), y: maxHeightCurveControl
        )
    )
    path.addLine(
        to: CGPoint(
            x: horizStart + (keyWidth * (1 + widthMultiplier)), y: vertStart - (keyHeight * 1.2)
        )
    )
...
func genAlternatesView(key: UIButton) {
    ...
    if let frame = key.superview?.convert(key.frame, to: nil) {
        ...
        if keysWithAlternatesLeft.contains(char) {
            ...
            alternatesShapeLayer.path =
                alternateKeysPathLeft(
            ...
            alternatesViewX =
                frame.origin.x + width
                    - CGFloat(width * numAlternates + (3.0 * numAlternates) + 2.0)
            alternatesShapeLayer.path =
                alternateKeysPathRight(
                    startX: frame.origin.x, startY: frame.origin.y,
                    keyWidth: width, keyHeight: key.frame.height, numAlternates: numAlternates
                ).cgPath
        }
