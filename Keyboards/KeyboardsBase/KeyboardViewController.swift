...
class KeyboardViewController: UIInputViewController {
    ...
    func conditionallyShowTopNumbersRow() {
        if DeviceType.isPhone {
            if let stackViewNum = stackViewNum {
                view.addConstraint(
                    NSLayoutConstraint(
                        item: stackViewNum, attribute: .height, relatedBy: .equal, toItem: nil,
        ...
        if !emojisToDisplay[0].isEmpty {
            ...
            if !emojisToDisplay[2].isEmpty, DeviceType.isPad {
                ...
                if UITraitCollection.current.userInterfaceStyle == .light {
                    ...
                    padEmojiDivider0.backgroundColor = UIColor(
                        cgColor: commandBarPlaceholderColorCG
                    )
                    padEmojiDivider1.backgroundColor = UIColor(
                        cgColor: commandBarPlaceholderColorCG
                    )
                }
                ...
                if UITraitCollection.current.userInterfaceStyle == .light {
                    ...
                    phoneEmojiDivider.backgroundColor = UIColor(
                        cgColor: commandBarPlaceholderColorCG
                    )
                }
                conditionallyHideEmojiDividers()
        ...
        if let documentContext = proxy.documentContextBeforeInput, !documentContext.isEmpty {
            if let inString = proxy.documentContextBeforeInput {
                ...
                if currentPrefix.hasPrefix("(") || currentPrefix.hasPrefix("#")
                    ...
                    currentPrefix = currentPrefix.replacingOccurrences(
                ...
                if proxy.selectedText != nil,
                   [.idle, .selectCommand, .alreadyPlural, .invalid].contains(commandState) {
                ...
                let completionOptions = LanguageDBManager.shared.queryAutocompletions(
                    word: currentPrefix
                )
    ...
    func getPronounAutosuggestions() {
        let prefix =
            proxy.documentContextBeforeInput?.components(separatedBy: " ").secondToLast() ?? ""
        ...
        for i in 0 ..< 3 {
            ...
            if let tense = pronounAutosuggestionTenses[prefix.lowercased()] {
                ...
                var suggestion = LanguageDBManager.shared.queryVerb(
                    of: verbsAfterPronounsArray[i], with: outputCols
                )[0]
    ...
    func getAutosuggestions() {
        var prefix =
            proxy.documentContextBeforeInput?.components(
                separatedBy: " "
            ).secondToLast() ?? ""
        ...
        if prefix.isNumeric {
        ...
        } else if ["English", "French", "German", "Spanish"].contains(controllerLanguage),
                  pronounAutosuggestionTenses.keys.contains(prefix.lowercased()) {
            ...
            let suggestionsLowerCasePrefix = LanguageDBManager.shared.queryAutosuggestions(
                of: prefix.lowercased()
            )
            let suggestionsCapitalizedPrefix = LanguageDBManager.shared.queryAutosuggestions(
                of: prefix.capitalized
            )
            ...
            if !suggestionsLowerCasePrefix[0].isEmpty {
                ...
                for i in 0 ..< 3 {
                    ...
                    if shiftButtonState == .shift {
                        ...
                        let nounForm = LanguageDBManager.shared.queryNounForm(
                            of: suggestionsLowerCasePrefix[i]
                        )[0]
    ...
    func conditionallySetAutoActionBtns() {
        ...
        if commandState == .idle {
            ...
            if autoAction0Visible {
                ...
                setBtn(
                    btn: translateKey,
                    color: firstCompletionIsHighlighted
                        ? keyColor.withAlphaComponent(0.5) : keyboardBgColor,
                    name: "AutoAction0",
                ...
                styleBtn(
                    ...
                    radius: firstCompletionIsHighlighted
                        ? commandKeyCornerRadius / 2.5 : commandKeyCornerRadius
                )
            ...
            if firstCompletionIsHighlighted, completionWords[1] == " ",
               completionWords[0] != currentPrefix {
            ...
            autoActionAnnotation(
                autoActionWord: !autoAction0Visible ? completionWords[0] : completionWords[1],
                index: 1,
                KVC: self
            )
            ...
            if autoAction2Visible, emojisToShow == .zero {
                ...
                autoActionAnnotation(
                    autoActionWord: !autoAction0Visible ? completionWords[1] : completionWords[2],
                    index: 2,
                    KVC: self
                )
                ...
                if DeviceType.isPhone {
                    pluralKey.titleLabel?.font = .systemFont(
                        ofSize: scribeKey.frame.height * scalarFontPhone
                    )
                } else if DeviceType.isPad {
                    pluralKey.titleLabel?.font = .systemFont(
                        ofSize: scribeKey.frame.height * scalarFontPad
                    )
                }
                activateBtn(btn: pluralKey)
                ...
                styleBtn(
                    btn: phoneEmojiKey0, title: emojisToDisplayArray[0],
                    radius: commandKeyCornerRadius
                )
                styleBtn(
                    btn: phoneEmojiKey1, title: emojisToDisplayArray[1],
                    radius: commandKeyCornerRadius
                )

                if DeviceType.isPhone {
                    phoneEmojiKey0.titleLabel?.font = .systemFont(
                        ofSize: scribeKey.frame.height * scalarFontPhone
                    )
                    phoneEmojiKey1.titleLabel?.font = .systemFont(
                        ofSize: scribeKey.frame.height * scalarFontPhone
                    )
                } else if DeviceType.isPad {
                    phoneEmojiKey0.titleLabel?.font = .systemFont(
                ...
                setBtn(
                    btn: padEmojiKey0, color: keyboardBgColor, name: "EmojiKey0",
                    canBeCapitalized: false,
                    isSpecial: false
                )
                setBtn(
                    btn: padEmojiKey1, color: keyboardBgColor, name: "EmojiKey1",
                    canBeCapitalized: false,
                    isSpecial: false
                )
                setBtn(
                    btn: padEmojiKey2, color: keyboardBgColor, name: "EmojiKey2",
                    canBeCapitalized: false,
                    isSpecial: false
                )
                styleBtn(
                    btn: padEmojiKey0, title: emojisToDisplayArray[0],
                    radius: commandKeyCornerRadius
                )
                styleBtn(
                    btn: padEmojiKey1, title: emojisToDisplayArray[1],
                    radius: commandKeyCornerRadius
                )
                styleBtn(
                    btn: padEmojiKey2, title: emojisToDisplayArray[2],
                    radius: commandKeyCornerRadius
                )

                padEmojiKey0.titleLabel?.font = .systemFont(
                    ofSize: scribeKey.frame.height * scalarEmojiKeyFont
                )
                padEmojiKey1.titleLabel?.font = .systemFont(
                    ofSize: scribeKey.frame.height * scalarEmojiKeyFont
                )
                padEmojiKey2.titleLabel?.font = .systemFont(
                    ofSize: scribeKey.frame.height * scalarEmojiKeyFont
                )
    ...
    func clearPrefixFromTextFieldProxy() {
        ...
        guard !currentPrefix.isEmpty, autoActionState != .suggest
        else {
            return
        }

        guard let documentContext = proxy.documentContextBeforeInput, !documentContext.isEmpty
        else {
            return
        }
    ...
    func executeAutoAction(keyPressed: UIButton) {
        ...
        if emojiAutoActionRepeatPossible
            && ((keyPressed == phoneEmojiKey0 || keyPressed == phoneEmojiKey1)
                || (keyPressed == padEmojiKey0 || keyPressed == padEmojiKey1
                    || keyPressed == padEmojiKey2)
        ...
        if [.idle, .selectCommand, .alreadyPlural, .invalid].contains(commandState) {
            if wordForWordDeletionIsEnabled(), longPressOnDelete {
                ...
                proxy.deleteBackward()
                  ...
                  !(allPrompts.contains(commandBar.text ?? "")
                      || allColoredPrompts.contains(commandBar.attributedText ?? NSAttributedString())) {
            guard let inputText = commandBar.text, !inputText.isEmpty
            else {
                return
            }
    ...
    func deleteWordBackward() {
        guard let documentText = proxy.documentContextBeforeInput
        else {
            return
        }
    ...
    func setCommandBtns() {
        setBtn(
            btn: translateKey, color: commandKeyColor, name: "Translate", canBeCapitalized: false,
            isSpecial: false
        )
        activateBtn(btn: translateKey)
        ...
        if controllerLanguage != "Indonesian" {
            setBtn(
                btn: conjugateKey, color: commandKeyColor, name: "Conjugate",
                canBeCapitalized: false,
                isSpecial: false
            )
            setBtn(
                btn: pluralKey, color: commandKeyColor, name: "Plural", canBeCapitalized: false,
                isSpecial: false
            )
            activateBtn(btn: conjugateKey)
            activateBtn(btn: pluralKey)
    ...
    func setKeywidth() {
        ...
        if isLandscapeView {
            if DeviceType.isPhone {
                letterKeyWidth =
                    (UIScreen.main.bounds.height - 5) / CGFloat(letterKeys[0].count)
                        * scalarLetterNumSymKeyWidthLandscapeViewPhone
                numSymKeyWidth =
                    (UIScreen.main.bounds.height - 5) / CGFloat(numberKeys[0].count)
                        * scalarLetterNumSymKeyWidthLandscapeViewPhone
            } else if DeviceType.isPad {
                letterKeyWidth =
                    (UIScreen.main.bounds.height - 5) / CGFloat(letterKeys[0].count)
                        * scalarLetterNumSymKeyWidthLandscapeViewPad
                if !usingExpandedKeyboard {
                    numSymKeyWidth =
                        (UIScreen.main.bounds.height - 5) / CGFloat(numberKeys[0].count)
                            * scalarLetterNumSymKeyWidthLandscapeViewPad
            ...
            letterKeyWidth =
                (UIScreen.main.bounds.width - 6) / CGFloat(letterKeys[0].count)
                    * scalarLetterNumSymKeyWidth
            numSymKeyWidth =
                (UIScreen.main.bounds.width - 6) / CGFloat(symbolKeys[0].count)
                    * scalarLetterNumSymKeyWidth
        }
    }
        ...
        for row in 0 ..< numRows {
            for idx in 0 ..< keyboard[row].count {
                ...
                if DeviceType.isPhone
                    && key == "a"
                    && ((commandState != .translate
                            && (["English", "Indonesian", "Italian", "Portuguese"].contains(
                                controllerLanguage
                            )
                ...
                // Special key styling.
                if key == "delete" {
                    let deleteLongPressRecognizer = UILongPressGestureRecognizer(
                        target: self, action: #selector(deleteLongPressed(_:))
                    )
                ...
                if key == "hideKeyboard" {
                    styleIconBtn(
                        btn: btn, color: keyCharColor, iconName: "keyboard.chevron.compact.down"
                    )
                }
                ...
                if key == "return" {
                    if [.translate, .conjugate, .plural].contains(commandState) {
                        styleIconBtn(
                            btn: btn, color: keyCharColor, iconName: "arrowtriangle.right.fill"
                        )
                    } else {
                        styleIconBtn(
                            btn: btn, color: keyCharColor, iconName: "arrow.turn.down.left"
                        )
                    }
                }
                ...
                if key == "return" && proxy.keyboardType == .webSearch
                    && ![.translate, .conjugate, .plural].contains(commandState) {
                    // Override background color from adjustKeyWidth for "search" blue for web searches.
                    styleIconBtn(
                        btn: btn, color: .white.withAlphaComponent(0.9),
                        iconName: "arrow.turn.down.left"
                    )
                    btn.backgroundColor = UIColor(
                        red: 0.0 / 255.0, green: 121.0 / 255.0, blue: 251.0 / 255.0, alpha: 1.0
                    )
                }
                        ...
                        ((UIScreen.main.bounds.width - 6.0)
                                - (CGFloat(
                                    usingExpandedKeyboard == true
                                        ? symbolKeys[0].count : numberKeys[0].count
                                )
                                    * numSymKeyWidth))
                            / (CGFloat(letterKeys[0].count)
                                - 1.0)
                ...
                if key == "shift" || key == spaceBar || key == languageTextForSpaceBar {
                    btn.addTarget(
                        self, action: #selector(keyMultiPress(_:event:)), for: .touchDownRepeat
                    )
                }
            }
        }
    ...
    func loadKeys() {
        ...
        if DeviceType.isPhone {
            translateKey.titleLabel?.font = .systemFont(
                ofSize: scribeKey.frame.height * scalarCommandKeyHeightPhone
            )
            conjugateKey.titleLabel?.font = .systemFont(
                ofSize: scribeKey.frame.height * scalarCommandKeyHeightPhone
            )
            pluralKey.titleLabel?.font = .systemFont(
                ofSize: scribeKey.frame.height * scalarCommandKeyHeightPhone
            ...
            translateKey.titleLabel?.font = .systemFont(
                ofSize: scribeKey.frame.height * scalarCommandKeyHeightPad
            )
            conjugateKey.titleLabel?.font = .systemFont(
                ofSize: scribeKey.frame.height * scalarCommandKeyHeightPad
            )
    ...
    @IBAction func executeKeyActions(_ sender: UIButton) {
        guard
            let originalKey = sender.layer.value(
                forKey: "original"
            ) as? String,
        ...
        if !(["EmojiKey0", "EmojiKey1", "EmojiKey2"].contains(originalKey)
    ...
    func showDynamicConjugationView(verb: String) {
        ...
        guard
            let cases = NavigationBuilder.buildConjugationCases(
                verb: verb,
    ...
    func showDynamicDeclensionView(preposition _: String) {
        // Remove any existing view.
        for child in children {
            if child is DynamicConjugationViewController {
                child.removeFromParent()
        ...
        guard let cases = NavigationBuilder.getDeclensionCases(language: language)
        else {
            return
        }
    ...
    @objc func keyMultiPress(_ sender: UIButton, event: UIEvent) {
        ...
        if let touches = event.allTouches, let touch = touches.first {
            ...
            if originalKey != "shift", proxy.documentContextBeforeInput?.count != 1,
               ![.translate, .conjugate, .plural].contains(commandState) {
                let charBeforeSpace = String(
                    Array(proxy.documentContextBeforeInput!).secondToLast()!
                )
            ...
            if touch.tapCount == 2,
               ...
               doubleSpacePeriodsEnabled() {
                ...
                } else if commandBar.text!.suffix(2) != "  ",
                          [.translate, .conjugate, .plural].contains(commandState) {
    ...
    @objc func deleteLongPressed(_ gesture: UIGestureRecognizer) {
        ...
        if let commandBarText = commandBar?.text,
           [.translate, .conjugate, .plural].contains(commandState),
           allPrompts.contains(commandBarText) {
    ...
    @objc func setAlternatesView(sender: UILongPressGestureRecognizer) {
        ...
        for char in alternateKeys {
            ...
            if char == alternateKeys.first, keysWithAlternatesLeft.contains(char) {
                setBtn(
                    btn: alternateKey, color: commandKeyColor, name: char, canBeCapitalized: true,
                    isSpecial: false
                )
                ...
                setBtn(
                    btn: alternateKey, color: keyColor, name: char, canBeCapitalized: true,
                    isSpecial: false
                )
            }
