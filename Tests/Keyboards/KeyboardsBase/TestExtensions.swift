...
class ExtensionTest: XCTestCase {
    func testSecondToLastNotNil() throws {
        let array = [1, 2, 3, 4, 5]

        let result = try XCTUnwrap(array.secondToLast())
...
extension ExtensionTest {
    func testSetColorForExistingText() {
        ...
        let range = (attributedString.string as NSString).range(
            of: textForAttribute, options: .caseInsensitive
        )

        attributedString.enumerateAttribute(.foregroundColor, in: range, options: []) {
            value, _, _ in
